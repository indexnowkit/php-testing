...
`OptionalPackageAssertions::assertDetected($checkOutput)` проверяет, что печатает `check` про `indexnowkit/sitemap`,
`indexnowkit/verify` и `indexnowkit/history`, когда адаптер оставляет предикаты на детект: строка
`<feature>: not installed` с командой установки для каждого действительно отсутствующего пакета и ни одной такой
строки для установленного. При `INDEXNOWKIT_OPTIONAL_PACKAGES=absent` (джоба CI `optional-packages-absent`, которая
физически удаляет три пакета) само отсутствие становится утверждением: адаптер, который грузит класс пакета, чтобы
узнать, установлен ли он, проходит с пакетом в `vendor/` и падает фаталом без него.
